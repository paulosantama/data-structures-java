...
public abstract class AbstractHeap<T extends Scorable> {
  ...
  private void heapify(int index) {
    ...
    if (left < this.getSize()
            && this.compareScore(this.arr.get(left).getScore(), arr.get(smallest).getScore())) {
      smallest = left;
    }

    if (right < this.getSize()
            && this.compareScore(this.arr.get(right).getScore(), arr.get(smallest).getScore())) {
      smallest = right;
    }
  ...
  private void rearrangeFromCurrent(int current) {
    while (current > 0 && this.compareScore(
            this.arr.get(current).getScore(),
            this.arr.get(parent(current)).getScore()
    )) {
